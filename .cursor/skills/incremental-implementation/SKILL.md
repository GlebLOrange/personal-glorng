...
Slice 1: Create a task (DB + API + basic UI)
    → Targeted tests pass, user can create a task via the UI

Slice 2: List tasks (query + API + UI)
    → Targeted tests pass, user can see their tasks

Slice 3: Edit a task (update + API + UI)
    → Targeted tests pass, user can modify tasks

Slice 4: Delete a task (delete + API + UI + confirmation)
    → Targeted tests pass, full CRUD complete
```
...
**Good:** Three separate commits — one for each change (when the user asks to commit).

### Rule 2: Keep It Compilable

After each increment, the project must build and existing tests for touched code must pass. Don't leave the codebase in a broken state between slices.
...
- [ ] Targeted tests for the slice pass (full suite only when user asks or fixing CI)
- [ ] The build succeeds (`npm run build` for frontend; `ruff check` for backend)
- [ ] Type checking passes (`npx tsc --noEmit` for frontend)
- [ ] Linting passes (`npm run lint` or `make lint-check`)
- [ ] The new functionality works as expected
- [ ] The slice is ready to commit when the user asks
