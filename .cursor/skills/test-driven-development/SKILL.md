...
**Related:** For browser-based changes, combine TDD with runtime verification using the browser MCP (snapshot, screenshot, console) after unit tests pass.
...
Run the changed test file after every refactor step to confirm nothing broke.
...
For UI changes, unit/component tests alone may not be enough — verify in the browser when the task involves layout or interaction.
...
5. VERIFY: Reload, screenshot, confirm console is clean; run targeted unit tests
```

Use the browser MCP (`browser_navigate`, `browser_snapshot`, `browser_take_screenshot`) for runtime verification. Do not run Playwright e2e unless the user asks or CI is failing.
