...
<template>
  <aside
    v-if="visible"
    :aria-label="DATE_TIME_LOCATION_SECTION"
    class="w-full border-b border-surface-border"
  >
    <RouterLink
      :to="TIME_DATE_WEATHER_LOCATION_PATH"
      class="interactive-surface block w-full px-4 py-4 md:py-4 rounded-none border-0 hover:border-accent-blue/40 transition-colors"
    >
      <div class="max-w-5xl mx-auto flex items-center justify-between gap-4 min-w-0">
        ...
        <span class="text-base text-surface-sage shrink-0">{{ DATE_TIME_LOCATION_SECTION }} →</span>
      </div>
    </RouterLink>
  </aside>
</template>
