...
Coding standards and agent behavior use a hybrid layout: thin always-on and path-triggered stubs in [`.cursor/rules/`](.cursor/rules/) (safety, git workflow, dependencies, backend FastAPI/Python, frontend Vue/Pinia/TypeScript, design system) that point at full guidance in [`.cursor/skills/`](.cursor/skills/) (`agent-safety`, `agent-git-workflow`, `dependencies`, `python-fast-api`, `vue-client`, `vue-pinia`, `toss-style-design-system`, `ui-ux-pro-max`, `saving-workspace-context`). This file covers environment and bootstrap only — do not commit ephemeral PR review dumps at the repo root (that anti-pattern lived as `BASEDIR.md`). Generic process skills (TDD, code-review, security hardening, etc.) live user-level under `~/.cursor/skills` / `~/.agents/skills` — install with `npx skills add <pkg>@<skill> -g -y`; keep project skills in `.cursor/skills/` only — do not commit `.agents/`, `.claude/`, or `skills-lock.json`.
...
Docs: published handbook at https://gleblorange.github.io/personal-glorng/ ; regenerate OpenAPI/inventory with `make docs-generate` after router or registry changes.
