...
async def test_cleanup_expired_shares_worker_invokes_service() -> None:
    mock_cleanup = AsyncMock(return_value={"deleted_rows": 2, "deleted_files": 2, "errors": 0})
    mock_session = AsyncMock()
    mock_session.__aenter__.return_value = mock_session
    mock_session.__aexit__.return_value = None

    with (
        patch("app.workers.tasks.get_session_factory", return_value=lambda: mock_session),
