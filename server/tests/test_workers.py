"""Tests for ARQ scheduling helpers."""
...
@pytest.mark.asyncio
@patch("app.workers.scheduling.enqueue_job", new_callable=AsyncMock)
async def test_schedule_reminder_enqueues_and_stores_job_id(
    mock_enqueue: AsyncMock,
    db: AsyncSession,
) -> None:
    mock_enqueue.return_value = "job-abc123"
    ...
    assert result.arq_job_id == "job-abc123"
...
@pytest.mark.asyncio
@patch("app.workers.scheduling.abort_job", new_callable=AsyncMock)
@patch("app.workers.scheduling.enqueue_job", new_callable=AsyncMock)
async def test_supersede_unsent_reminders_aborts_and_deletes(
    ...
    mock_abort.assert_awaited_once_with("job-old")
