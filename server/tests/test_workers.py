...
async def test_schedule_reminder_enqueues_and_stores_job_id(
    ...
    result = await schedule_reminder(registry, reminder)
...
async def test_supersede_unsent_reminders_aborts_and_deletes(
    ...
    old = await create_reminder(
        registry, task_id=task.id, remind_at=old_at, job_id="job-old"
    )
    keep = await create_reminder(registry, task_id=task.id, remind_at=new_at)

    await supersede_unsent_reminders(registry, task.id, exclude_id=keep.id)
    ...
    remaining = await registry.tasks.get_reminder(keep.id)
    assert remaining is not None
    assert await registry.tasks.get_reminder(old.id) is None
